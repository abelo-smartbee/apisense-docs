# Samples — "Honey bee colony health, Spring 2026" program

A seasonal honey bee health monitoring program run by Apisense in collaboration with the University of Life Sciences in Lublin. This page contains the complete set of sample collection protocols and colony inspection guidelines.

!!! warning "Read before starting"
    Before collecting samples, assessing *Varroa destructor* infestation, and performing inspections, **read this entire instruction**.

## Protocols

| | Protocol | Status |
|---|---|---|
| 1 | [*Varroa destructor* monitoring](protocol-1-varroa.md) — sugar roll + natural drop on sticky board | Mandatory |
| 2 | [Live bee collection](protocol-2-live-bees.md) — ~40 workers from the brood nest | Mandatory |
| 3 | [Brood comb section](protocol-3-brood-comb.md) — 10×10 cm with affected brood | Conditional (contact first) |
| 4 | [Dead bees](protocol-4-dead-bees.md) — bottom-board collection | Recommended |
| 5 | [Honey sample](protocol-5-honey.md) — one tube of super honey per sensor | Mandatory |

Additionally: [Colony inspection](colony-inspection.md) — required frame photos and observation list to record in the Apisense app.

## Contact

### Beekeeper coordinators

| | |
|---|---|
| **Daniel Bajorek** | +48 455 506 328 · [daniel.bajorek@apisense.ai](mailto:daniel.bajorek@apisense.ai) |
| **Olga Janczak** | +48 455 505 069 · [olga.janczak@apisense.ai](mailto:olga.janczak@apisense.ai) |

### Sample shipping / scientific supervision

**Dr Maciej Bryś**
University of Life Sciences in Lublin
ul. Doświadczalna 54, 20-280 Lublin, Poland
+48 455 501 920 · [maciej.brys@apisense.ai](mailto:maciej.brys@apisense.ai)

### Technical help and support

[bee@apisense.ai](mailto:bee@apisense.ai)

## Shipping samples {#shipping-samples}

1. Place all labeled samples (with examination codes generated in the Apisense app) together in a **cardboard box**.
2. Ship on a **business day (Monday–Thursday)** to avoid the package sitting over the weekend.
3. Use **Poczta Polska** (estimated cost PLN 20–30) **or a courier service** so the samples reach the lab as quickly as possible.

!!! tip "Live bees"
    Live bee samples require special attention when packing and shipping — see [Protocol 2 — live bees](protocol-2-live-bees.md#shipping). Shipments of live bees via Poczta Polska may only be sent by apiary owners located in Poland.

**Shipping address:**

```
Dr Maciej Bryś
Uniwersytet Przyrodniczy w Lublinie
ul. Doświadczalna 54
20-280 Lublin
Poland
tel. +48 455 501 920
maciej.brys@apisense.ai
```
