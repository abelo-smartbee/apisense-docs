# Do pobrania

Dokumenty techniczne i materiały dotyczące urządzeń Apisense.

______________________________________________________________________

## Rysunki techniczne

Rysunki montażowe z wymiarami — przydatne przy planowaniu instalacji oraz integracji z istniejącą infrastrukturą pasieki.

- **Apisense Hub** — <a href="files/Hub_Asm.pdf">rysunek techniczny (PDF)</a>
- **Apisense VitalSensor** — <a href="files/SENSOR_ASM.pdf">rysunek techniczny (PDF)</a>

<!-- TODO: dodać rysunek techniczny Apisense Scale -->

______________________________________________________________________

## Pliki do druku 3D — VitalSensor

Niektóre elementy obudowy VitalSensora bywają uszkadzane podczas transportu. Udostępniamy modele 3D (STEP), żeby można było samodzielnie wydrukować części zamienne.

- **Apisense VitalSensor** — <a href="files/VitalSensor_3D_print_files.zip">modele 3D do druku (ZIP, STEP)</a>

______________________________________________________________________

## Instrukcja montażu

Ilustrowana instrukcja krok po kroku — w stylu IKEA.

- **Apisense Box** — <a href="/assembly/">instrukcja montażu online</a> · <a href="files/Apisense_Box_Assembly_Instruction.pdf">wersja PDF</a>
